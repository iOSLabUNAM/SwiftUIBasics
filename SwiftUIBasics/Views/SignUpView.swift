...
class SignUpViewModel: ObservableObject {
    ...
    @Published var isValidePassword: Bool = false
    ...
    init() {
        ...
        Publishers.CombineLatest4($isValidPasswordMatch, $isValidPasswordNumber, $isValidPasswordSymbol, $isValidPasswordLowerCase )
            .map { (a, b, c, d) in
                return a && b && c && d
            }
            .assign(to: \.isValidePassword, on: self)
            .store(in: &cancelableSet)

        Publishers.CombineLatest4($isValidEmail, $isValidPasswordLength, $isValidPasswordUpperCase, $isValidPasswordMatch)
            .map { (a, b, c, d) in
                return a && b && c && d
            }
...
struct SignUpView: View {
    ...
    var body: some View {
        VStack {
            ...
            RequirementText(text: "Email is valid", isValid: vm.isValidEmail)
                .padding()
            ...
            VStack {
                ...
                RequirementText(text: "Caracter special", isValid: vm.isValidPasswordSymbol)
            }
            .padding()
